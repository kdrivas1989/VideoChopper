#!/usr/bin/env python3
"""Video Chopper Web App - Upload videos, select trim times, and download results."""
...
def trim_video():
    try:
        ...
        # Support both single segment (legacy) and multi-segment formats
        ...
        # Keep last output for legacy single-download compatibility
        if outputs:
...
def download_video(video_id, segment=None):
    ...
    # Handle multi-segment downloads
    if segment is not None and 'outputs' in video:
        # Find the specific segment output
...
def save_to_folder():
    """Save trimmed video files to a specified folder."""
    import shutil
    data = request.json
    video_id = data.get('id')
